...
subroutine  CartToInternal(Xcart,int_arr)
    implicit none
    ...
    real(8),intent(in)::Xcart(0:Ncarts-1)
    ...
    do i = 1,Natoms-1
            ...
            if (i==1) then
                    ...
                    r1 = Xcart(3*atm1:3*atm1+2)
                    r2 = Xcart(3*atm2:3*atm2+2)
                    v1 = r2-r1
                    nr1  = norm2(v1)
                    int_arr(b_ctr) = nr1
                    b_ctr =b_ctr + 1
...
subroutine getRotMat(M,theta,u)
    implicit none
...
!*************************************************************************************!
subroutine getNorm(Ndim,v,norm)
    implicit none
    ...
    enddo
    norm = dsqrt(sum1)
endsubroutine getNorm
!*************************************************************************************!
subroutine getCrossProd(v1,v2,v_cross,Ndim)
    implicit none
    integer,intent(in)::Ndim
...
endsubroutine getCrossProd
!*************************************************************************************!
subroutine dihToCart(u1,u2,r2,a2,d2,q4)
    implicit none
...
subroutine IntToCart(S,Xcart)
    ...
    real(8),intent(out),dimension(0:Ncarts-1)::Xcart
    ...
    do i = 0,Natoms-1
        if (atom_ind(i,1)==0) then
            ...
            !Place the second atom along Z-axis
            Xcart(3*i:3*i+1) = 0.0
            Xcart(3*i+2)  = B(b_ctr)
            b_ctr = b_ctr + 1
            ...
            y = dsqrt(b2*b2 - z*z)
            !Place the 3rd atom in the yz plane
            Xcart(3*i) = 0.d0
            Xcart(3*i+1) = y
            Xcart(3*i+2) = z
            b_ctr =  b_ctr +1
            ...
            u1   = Xcart(3*atm2:3*atm2+2)-Xcart(3*atm1:3*atm1+2)
            u2   = Xcart(3*atm3:3*atm3+2)-Xcart(3*atm2:3*atm2+2)
            !Get the cartesian coordinate with the 3rd atom as the origin.
            call dihToCart(u1,u2,r2,a2,d2,q)

            !Compute the absolute cartesian position
            q   = q + Xcart(3*atm3:3*atm3+2)
            Xcart(3*i:3*i+2) = q(:)
        endif
    enddo
...
!****************************************************************************************#
subroutine dtrig_series_odd(sinth,costh,Ncoeff,coeff_arr,stride_arr_odd,fn_order_odd,dfc_dtors)
    !"""
    !dfc_dtors   : An array of length equal to the no. of fixed internals(len(fix_tors)). i'th element contains the 
    !          derivatives of  input internal w.r.t the i'th fixed coordinate.
    !theta   : An array of length equal to the no. of fixed internals. i'th element contains the value of the 
    !          i'th fixed coordinate.
    !idx     : An array of length equal to the no. of fixed internals. i'th element contains the value of the 
    !"""
    implicit none 
    integer,intent(in):: fn_order_odd
    integer,intent(in)::Ncoeff(0:1),stride_arr_odd(0:fn_order_odd-1,0:1)
    real(8),intent(in):: sinth(0:n_fix_tors-1,0:max_id(0)),costh(0:n_fix_tors-1,0:max_id(0))
    real(8),intent(in):: coeff_arr(0:fn_order_odd-1)
    real(8),intent(out)::dfc_dtors(0:n_fix_tors-1)
    real(8)::dfdt(0:n_fix_tors-1)
    integer:: jj,i,j
    ...
    dfc_dtors = 0.d0 
    dfdt =  0.d0
    do jj = 0,fn_order_odd-1
        i = stride_arr_odd(jj,0);j=stride_arr_odd(jj,1)
        if (jj < Ncoeff(0)) then
            call get_cs_deriv(i,j,sinth,costh,coeff_arr(jj),dfdt) 
            dfc_dtors(:) = dfc_dtors(:) + dfdt(:)
        else
            call get_sc_deriv(i,j,sinth,costh,coeff_arr(jj),dfdt) 
            dfc_dtors(:) = dfc_dtors(:) + dfdt(:)
        endif
    enddo
end subroutine
...
!****************************************************************************************#
subroutine trig_series_odd(sinth,costh,Ncoeff_odd,coeff_arr,stride_arr_odd,fn_order_odd,fit_val)
    implicit none 
    integer,intent(in):: fn_order_odd
    integer,intent(in)::Ncoeff_odd(0:1),stride_arr_odd(0:fn_order_odd-1,0:1)
    real(8),intent(in):: sinth(0:n_fix_tors-1,0:max_id(0)),costh(0:n_fix_tors-1,0:max_id(0)),coeff_arr(0:fn_order_odd-1)
    real(8),intent(out)::fit_val
    integer:: jj,i1,j1
    fit_val = 0.d0
    do jj = 0,fn_order_odd-1
        i1 = stride_arr_odd(jj,0); j1 = stride_arr_odd(jj,1)
        if (jj<Ncoeff_odd(0)) then 
            fit_val = fit_val +  coeff_arr(jj) * costh(0,i1) * sinth(1,j1)
        else         
            fit_val = fit_val +  coeff_arr(jj) * sinth(0,i1) * costh(1,j1) 
        endif
    enddo
end subroutine
!****************************************************************************************#
...
subroutine  get_dsdx(X,dsdx)
    ...
    !call CartToInternal(X,S)
    do i =0,Ncarts-1
        dS_ar = 0.d0
        Xtmp =X
        do istep = -2,2
            Xtmp(i) = X(i) + dble(istep) * dx
            call CartToInternal(Xtmp,Stmp)
            dS_ar(istep+2,:)   = Stmp(:)
        enddo
...
endsubroutine 
!****************************************************************************************#
subroutine getCSarr(fix_tors,max_id,sinth,costh)
    implicit  none
    integer:: i,j,k
...
subroutine IntToCartGrad(dsdx,dVds,dVdx)
    implicit none 
    integer::i 
...
endsubroutine 
!****************************************************************************************#
subroutine getPot(X,pot,Vt_out)
    implicit none 
   integer::i
    real(8),intent(in):: X(0:Ncarts-1)
    ...
    call getCSarr(fix_tors,max_id,sinth,costh)
    ...
    call  ComputePot(S_diff,sinth,costh,fc_quad_even,fc_quad_odd,fc_qubic_even_gt5,fc_qubic_even_lt5,fc_qubic_even_lt2,fc_qubic_odd_gt5,fc_qubic_odd_lt5,fc_qubic_odd_lt2,fc_quartic_even_gt5,fc_quartic_even_lt5,fc_quartic_even_lt2,fc_quartic_odd_gt5,fc_quartic_odd_lt5,fc_quartic_odd_lt2,fc_quartic_even_ltpt1,fc_quartic_odd_ltpt1,fc_qo_even,fc_qo_odd,fc_qo_ang_even,pot,Vt_out)
    pot = pot * CminvToHartee
    Vt_out = Vt_out * CminvToHartee
endsubroutine 
!****************************************************************************************#
subroutine getForce(X,dVdx,pot)
    implicit none 
    integer::i
    real(8),intent(in):: X(0:Ncarts-1)
    ...
    call get_dsdx(X,dsdx)
    call IntToCartGrad(dsdx,dVds,dVdx)
    dVdx = -dVdx * CminvToHartee 
    call  ComputePot(S_diff,sinth,costh,fc_quad_even,fc_quad_odd,fc_qubic_even_gt5,fc_qubic_even_lt5,fc_qubic_even_lt2,fc_qubic_odd_gt5,fc_qubic_odd_lt5,fc_qubic_odd_lt2,fc_quartic_even_gt5,fc_quartic_even_lt5,fc_quartic_even_lt2,fc_quartic_odd_gt5,fc_quartic_odd_lt5,fc_quartic_odd_lt2,fc_quartic_even_ltpt1,fc_quartic_odd_ltpt1,fc_qo_even,fc_qo_odd,fc_qo_ang_even,pot,Vt_out)
endsubroutine 
!****************************************************************************************#
subroutine numGrad(X,dVdx_num)
implicit none 
real(8),intent(in)::X(0:Ncarts-1)
